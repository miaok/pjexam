  ...
  useEffect(() => {
    ...
    if (!hasRestoredRef.current && (quizMode === 'exam' || quizMode === 'practice' || quizMode === 'blind')) {
      ...
      if (quizMode === 'blind') {
        hasValidProgress =
          saved &&
          saved.baijiuQuestions &&
          Array.isArray(saved.baijiuQuestions) &&
          saved.baijiuQuestions.length > 0 &&
          typeof saved.currentBaijiuIndex === 'number' &&
          (saved.currentBaijiuIndex > 0 ||
            (saved.baijiuUserAnswer && Object.values(saved.baijiuUserAnswer).some(v => v && (Array.isArray(v) ? v.length > 0 : v !== '')))
          );
      } else {
        hasValidProgress =
          saved &&
